...
func TestInstallSkillOverwritesAFileFromBeforeTheMarker(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	dir, _ := skillDir(false)
	...
	if _, err := installSkill(dir, false); err != nil {
		t.Fatalf("installSkill refused a file an older rmine wrote: %v", err)
	}
	got, _ := os.ReadFile(path)
	if string(got) != string(stampedSkill()) {
		t.Error("install did not replace the older generated file")
	}
}
